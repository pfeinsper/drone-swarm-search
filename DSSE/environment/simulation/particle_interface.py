...
class ParticleInterface:
    ...
    def __init__(self, dataset_path: str, group_key: str, time_step: timedelta, cell_size: float = 45,
                 pre_render_time:timedelta=timedelta(microseconds=0), square_matrix:bool=True):
        """
        A class for interfacing with the .h5 dataset and turning it into a matrix of probabillities of containment.
        ...
        self.elapsed_time = pre_render_time # how much time has passed in the simulation
        ...
        try:
            with h5py.File(self.dataset_path, 'r') as f:
                ...
                self.particle_sim_time_delta = timedelta(microseconds=int(self.particle_sim_time_delta.split(' ')[0][2:]) / 1000)
                self.particle_sim_time_steps = int(round((pre_render_time) / self.particle_sim_time_delta)) # how many time steps have passed for the particle simulation. Essentially, which frame of the particle trajectory are we on
        ...
        self.x_vals = EARTH_MEAN_RADIUS * long / 180 * pi * np.cos(south_west_ref[0] / 180 * pi)
        ...
        for time_step in range(self.x_vals.shape[1]):
            for particle_num in range(self.x_vals.shape[0]):
                row = int((self.y_len - self.y_vals[particle_num][time_step]) / self.y_len * r)
    ...
    def particle_matrix_with_interpolation(self):
        ...
        proportion = (self.elapsed_time - self.particle_sim_time_steps * self.particle_sim_time_delta) / self.particle_sim_time_delta
        # Interpolates between the two particle postitions at the current time step and the next one
        x_vals_interpol = self.x_vals[:,self.particle_sim_time_steps] * (1 - proportion) + self.x_vals[:,self.particle_sim_time_steps + 1] * proportion
    ...
    def step(self, indices=[], pod=0.75) -> tuple[np.ndarray, float, bool]:
        ...
        self.elapsed_time = min(self.pre_render_time + self.env_time_steps * self.env_time_step, (self.x_vals.shape[1] - 1) * self.particle_sim_time_delta)
        if self.elapsed_time - self.particle_sim_time_steps * self.particle_sim_time_delta >= self.particle_sim_time_delta:
            if self.particle_sim_time_steps < len(self.particle_matrix) - 1:
                self.particle_sim_time_steps += int(round((self.pre_render_time + self.env_time_steps * self.env_time_step - self.particle_sim_time_steps * self.particle_sim_time_delta) / self.particle_sim_time_delta))
        
        if self.elapsed_time == (self.x_vals.shape[1] - 1) * self.particle_sim_time_delta:
            sim_over = True
        ...
        if self.particle_sim_time_steps < len(self.particle_matrix) - 1:
            ...
            self.cur_particle_matrix = self.particle_matrix[self.particle_sim_time_steps]
    ...
    def draw_heatmap(self, search_random=0, search_greedy=0, pod=0.75):
        ...
        def on_key(event):
            if event.key == ' ':  # Spacebar
                ...
                if search_random > 0:
                    rows = np.random.randint(0, len(self.particle_matrix[self.particle_sim_time_steps]), size=(search_random))
                    cols = np.random.randint(0, len(self.particle_matrix[self.particle_sim_time_steps][0]), size=(search_random))
    ...
    def search_grid(self, pod: float, searched_cells: list[tuple[int]]) -> float:
        ...
        for cell in numbered_searched_cells:
            ...
            for particle_id in self.cur_particle_matrix[row][col]:
                self.particle_probabillities[particle_id] *= (1 - pod) ** num_drones # pod is probabillity of detection given the victim is there
                self.particle_probabillities_no_bayes[particle_id] *= (1 - pod) ** num_drones
        
        self.particle_probabillities /= sum(self.particle_probabillities)
        if sum(self.particle_probabillities) == 0:
            print("All cells have been searched with complete certainity, so bayesian updates cannot be made")
            raise AssertionError

        print(f"Prob Found During Operation: {1 - sum(self.particle_probabillities_no_bayes)}")

        return 1 - sum(self.particle_probabillities_no_bayes) # probabillity that the victim has been found
