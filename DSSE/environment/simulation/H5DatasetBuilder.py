...
class H5DatasetBuilder:
    def __init__(self, dataset_path: str, 
        ...
        if self.outfile_img_folder is not None and not self.outfile_img_folder.endswith('/'):
    ...
    def establish_connection(self) -> None:
        ...
        if not self.established:
            sim = DynamicParticleSimulation(random.uniform(14, 32), random.uniform(-180, -146), datetime.now(), particle_amount=500, object_types=[1, 13, 55])
            # Established connection to Copernicus Marine Service
            sim.init(username=self.username, password=self.password, use_env=self.use_env)
            self.simulation = sim
    ...
    def check_img_folder(self) -> bool:
        return os.path.exists(self.outfile_img_folder) if self.outfile_img_folder is not None else True
    
    def add_one_example(self, disaster_lat: float, disaster_long: float, start_time: datetime, duration_hours: int = 10, 
                        loglevel: int = 20, particle_amount: int = 50_000, particle_radius: int = 1000, 
                        object_types: list = [1], time_step=timedelta(hours=1), time_step_output=timedelta(hours=1), 
                        wind_uncertainity: float = 0.1, current_uncertainity: float = 0.1, 
                        horizontal_diffusion: float = 10, save_img=True, name="") -> None:
        ...
        if not os.path.exists(self.dataset_path):
            raise FileNotFoundError(f"Dataset {self.dataset_path} does not exist. Please create it first.")
        
        # Check if the output folders exist, if not, raise an error
        if not os.path.exists(self.outfile_nc_folder):
            raise FileNotFoundError(f"Output folder {self.outfile_nc_folder} does not exist. Please create it first.")
        
        # Raising an error if save-img is True but outfile_img_folder is None
        if save_img and self.outfile_img_folder is None:
            raise ValueError("You cannot save images if outfile_img_folder is None. Set save_img to False or set outfile_img_folder to a valid folder path.")
        if self.outfile_img_folder is not None and not os.path.exists(self.outfile_img_folder):
            raise FileNotFoundError(f"Output folder {self.outfile_img_folder} does not exist. Please create it first.")
        
        outfile_nc = os.path.abspath(f"{self.outfile_nc_folder}trj_{name}_{self.cur_index}.nc")
        # Exiting this function if the outfile_nc already exists
        if os.path.exists(outfile_nc):
            print(f"File {outfile_nc} already exists. Cannot simulate because it would overwrite the file.")
            print("Please delete the file or change the name of this example.")
            return
        
        # Check if the example already exists in the dataset
        if self.check_example_exists(disaster_lat, disaster_long, start_time, duration_hours, 
                    particle_amount, particle_radius, object_types, time_step, time_step_output,
                    wind_uncertainity, current_uncertainity, horizontal_diffusion):
            print(f"Example with disaster_lat={disaster_lat}, disaster_long={disaster_long}, start_time={start_time}, duration_hours={duration_hours}, particle_amount={particle_amount}, particle_radius={particle_radius}, object_types={object_types}, time_step={time_step}, time_step_output={time_step_output}, wind_uncertainity={wind_uncertainity}, current_uncertainity={current_uncertainity}, horizontal_diffusion={horizontal_diffusion} already exists in the dataset. Skipping...")
            return
        ...
        outfile_img = os.path.abspath(f"{self.outfile_img_folder}trj_{name}_{self.cur_index}.jpg") if save_img else None
        ...
        self.simulation.simulate(outfile_nc=outfile_nc, outfile_img=outfile_img, plot=save_img)
        ...
        # data conversion
        try:
            with h5py.File(self.dataset_path, 'r+') as f:  # Opens existing file and raises an error if it does not exist
                if f"example_{name}_{self.cur_index}" not in f:  # Check if group exists
                    ...
                    grp.create_dataset("start time", data=np.datetime_as_string(ds["status"]["time"][0].values, unit='s'), dtype=dt)
                    grp.create_dataset("time delta", data=str(ds["status"]["time"][1].values - ds["status"]["time"][0].values), dtype=dt)
                    ...
                    grp.create_dataset("time_step_output", data=str(time_step_output), dtype=dt)
                    grp.create_dataset("wind_uncertainity", data=wind_uncertainity)
                    grp.create_dataset("current_uncertainity", data=current_uncertainity)
                    grp.create_dataset("horizontal_diffusion", data=horizontal_diffusion)
                    ...
                    print(f"Added example_{name}_{self.cur_index} to {self.dataset_path} & deleted {outfile_nc}")
                else: 
    ...
    def check_example_exists(self, disaster_lat: float, disaster_long: float, start_time: datetime, duration_hours: int, 
                        particle_amount: int, particle_radius: int, 
                        object_types: list, time_step, time_step_output, 
                        wind_uncertainity: float, current_uncertainity: float, 
                        horizontal_diffusion: float) -> bool:
        ...
        with h5py.File(self.dataset_path, 'r') as f:
            for key in f.keys():
                ...
                grp = f[key]
                try:
                    if (grp["disaster_lat"][()] == disaster_lat and
                        grp["disaster_long"][()] == disaster_long and
                        grp["start time"][()].decode() == np.datetime_as_string(np.datetime64(start_time), unit='s') and
                        grp["duration_hours"][()] == duration_hours and
                        grp["particle_amount"][()] == particle_amount and
                        grp["particle_radius"][()] == particle_radius and
                        np.array_equal(grp["object_types"][()], np.array(object_types)) and
                        grp["time_step"][()].decode() == str(time_step) and
                        grp["time_step_output"][()].decode() == str(time_step_output) and
                        grp["wind_uncertainity"][()] == wind_uncertainity and
                        grp["current_uncertainity"][()] == current_uncertainity and
                        grp["horizontal_diffusion"][()] == horizontal_diffusion):
                            return True 
                except KeyError:
                    continue
        return False
    ...
    def clear_nc_folder(self) -> None:
        ...
        """
        if not os.path.exists(self.outfile_nc_folder):
            print(f"Output folder {self.outfile_nc_folder} does not exist. Nothing to clear.")
            return
        
        if not os.listdir(self.outfile_nc_folder):
            print(f"Output folder {self.outfile_nc_folder} is already empty. Nothing to clear.")
            return
        ...
        for file in os.listdir(self.outfile_nc_folder):
            if file.endswith(".nc"):
                ...
                if name == "unknown":
                    raise ValueError("File name does not contain an example name. Please use a valid file name.")
                ...
                with h5py.File(self.dataset_path, 'r+') as f:  # Opens existing file and raises an error if it does not exist
                    if f"example_{name}" not in f:  # Check if group exists
                        ...
                        # Delete the NetCDF file after adding it to the HDF5 dataset
                        os.remove(outfile_nc)
                        print(f"Deleted {file} from {self.outfile_nc_folder} and moved to {self.dataset_path} as example_{name}")
                    else: 
