...
class DynamicParticleSimulation:
    def __init__(
        ...
        if start_time.year < 2022:
            raise ValueError("Start time must be greater than or equal to 2022, as the " \
    ...
    def init(self, username=None, password=None, use_env=True):
        if use_env:
            if username is not None or password is not None:
                raise ValueError("Username and password must not be provided if use_env is True")
            ...
            if username is None or password is None:
                raise ValueError("Username and password must be provided if use_env is False")

        ds = copernicusmarine.open_dataset(dataset_id='cmems_mod_glo_phy_anfc_merged-uv_PT1H-i', chunk_size_limit=0, username=username, password=password)
        ...
        self.reader_winds = Reader("https://pae-paha.pacioos.hawaii.edu/thredds/dodsC/ncep_global/NCEP_Global_Atmospheric_Model_best.ncd", name="Wind Reader")
    
    def simulate(self, outfile_img=None, plot=False, outfile_nc=os.path.abspath("DSSE/environment/TrajectoryDatasets/sar_sim.nc")):
        l = Leeway(loglevel=100)
        print(Leeway.required_variables)
        try:
            l.add_reader(self.reader_currents, variables=['x_sea_water_velocity', 'y_sea_water_velocity'])
        ...
        for object in self.object_types:
            ...
            l.seed_elements(lon=self.disaster_long, lat=self.disaster_lat, time=self.start_time, object_type=object, **{'number': self.particle_amount, 'radius': self.particle_radius})        
        ...
        l.run(duration=timedelta(hours=self.duration_hours), time_step=self.time_step,
               time_step_output=self.time_step_output, outfile=outfile_nc, export_variables=["object_type"]) # outfile records required variables like lat, long, id, and status
...
if __name__ == "__main__":
    ...
    for i in range(4, 5):
        outfile_nc = os.path.abspath(outfile_nc_folder + "test_trj_one_particle" + str(i) + ".nc")
        outfile_img = os.path.abspath(outfile_img_folder + "test_trj_one_particle" + str(i) + ".jpg")
        ...
        sim = DynamicParticleSimulation(lat, long, dis_time, particle_amount=100, object_types=[1], particle_radius=0)
        sim.init()
