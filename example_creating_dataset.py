...
dataset_builder = H5DatasetBuilder("DSSE/environment/TrajectoryDatasets/sample_dataset.h5",
...
events = [("HurricaneFranklinTropicalStorm", 14, -65.2, datetime(2023, 8, 20, 12)),
...
for i, (name, lat, long, event_time) in enumerate(events):
    print(f"Adding example {i + 1}: {name} at ({lat}, {long}) on {event_time.isoformat()}")
    ...
    dataset_builder.add_one_example(
        ...
        object_types=[1, 13, 55], # The object types to simulate found in the Leeway drift model. 1 is a person in water, 13 is a life raft, and 55 is a fishing vessel.
        time_step=timedelta(minutes=30), # This is the time between calculations in the computer model. 
        time_step_output=timedelta(minutes=30), # This is how often an output is written to a file. For example, the model can calculate with 15-minute timesteps, but only report every hour
        ...
        save_img=i % 2 == 0,  # Save images for every second example. Note that this take a lot of time, even more that just the simulation.
        name=name
    )
